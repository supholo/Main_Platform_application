// src/components/ConfigurationManagement/AuditLog.tsx
...
export const AuditLog: React.FC<AuditLogProps> = ({
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((log) => {
      // Fix searchTerm matching with optional chaining
      const matchesSearch =
      ...
      // Fix time range checking with type assertion
  ...
  const exportLogs = () => {
    const exportData = filteredLogs.map((log) => ({
      ...
      configuration: configurations.find((c) => c.id === log.configurationId)
        ?.name,
      details: log.details,
